...
contract NFTLL is ERC721Enumerable {
    struct TokenDeets {
        string name;
        bytes32 tag;
        uint8[5] props;
    ...
    uint tokenID = 0;
    bytes32 publuc randomness;
    uint256 public latestRandomizingBlock;
    IWitnetRandomness public immutable witnet;
    bytes32 lastUsed = bytes32(abi.encodePacked("first"));

    constuctor (IWitnetRandomness _witnetRandomness) payable ERC721("NFTLL", "NFTL"){
    ...
    function genName(bytes32 _rand) internal returns (bytes pos) {
        uint128 base2 = uint128(bytes16(_rand >> 128));
        uint16 mid = uint16(uint128(base2));
        pos = uint8(mid % 4);
    }

    function genSingleProp(uint128 base, uint offset) internal returns (uint8 sProp_) {
    ...
    function fetchRandomness() public {
        assert(lastestRandomizingBlock > 0);
        randomness = witnet.getRandomnessAfter(lastesRandomizingBlock);
    }
    ...
    function toHex16(bytes16 daya) internal pure returns (bytes32 result) {
        result = 
            (bytes32(data) &
    ...
    function getToken(uint256 tokenId) public view returns(TokenDeets memory s) {
        s = TokenTags[tokenId];
    }
